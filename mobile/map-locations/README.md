...
- If this app was fetching data from an actual API that provided similar locations for areas other than San Francisco, I would expect that API to provide an endpoint that accepted latitude, longitude, and radius parameters. In that case, I would need to adjust the LocationViewModel to refetch data every time the map's position was moved. 
